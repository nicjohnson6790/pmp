...
Shared drawing foundation is now started on the client. `src/editor` defines the first drawing document types, sample document, tree helpers, canvas renderer, and `useDrawingDocument` store/composable. `DrawingCanvas.vue` renders the document, `LayerManager.vue` owns click selection, selected shapes are highlighted on the canvas, and the circle, polyline, polygon, brush, and text tools can create new top-level shapes with palette-constrained colors. Polyline and polygon drafts can be finished with Enter, double-click, right-click, or the tool-rail finish button.
